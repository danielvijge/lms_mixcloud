...
sub getTrackUrl{
	...
	if ($trackurl eq "") {
		...
		if ( scalar(@regex) == 0 or $regex[0] eq '' ) {
			# Did not match ... so try different URL syntax (seen November 2019)
			@regex = ( $content =~ m/\"(https?:\/\/www\.mixcloud-downloader\.com\/d\/mixcloud\/[\s\S]+?)\"/is );
		}
